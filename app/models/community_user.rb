class CommunityUser < ApplicationRecord
  ...
  def post_score
    Rails.cache.fetch("privileges/#{id}/post_score", expires_in: 3.hours) do
      ...
      good_posts = Post.by(user).where('score > 0.5').where.not(post_type_id: exclude_types).count
      bad_posts = Post.by(user).where('score < 0.5').where.not(post_type_id: exclude_types).count
