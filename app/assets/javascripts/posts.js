$(() => {
  ...
  $('.post-field').on('keyup change', evt => {
      evt.preventDefault();
      const $tgt = $(evt.target);
      const content = $tgt.val();
      const caretPos = content.length;

      const bodyCharCount = document.getElementById("body-char-count");
      const bodyCharIcon = document.getElementById("body-char-count__icon");
      const bodyChar = document.getElementById("body-char");
      const savePostBtn = document.getElementById('commit');
      bodyCharCount.innerText = caretPos + '/30000';

      if (caretPos > 29950 && caretPos <= 29999){
          savePostBtn.disabled = false;
          bodyChar.style.display = "block";
          bodyCharIcon.classList.remove('fa-ellipsis-h');
          bodyCharIcon.classList.remove('fa-times');
          bodyCharIcon.classList.remove('has-color-red-500');
          bodyCharIcon.classList.add('fa-exclamation-circle');
          bodyChar.classList.add('has-color-yellow-700');
      }
      else if (caretPos >= 30000){
          savePostBtn.disabled = true;
          bodyChar.style.display = "block";
          bodyCharIcon.classList.remove('fa-ellipsis-h');
          bodyCharIcon.classList.remove('fa-exclamation-circle');
          bodyCharIcon.classList.add('fa-times');
          bodyChar.classList.remove('has-color-yellow-700');
          bodyChar.classList.add('has-color-red-500');
      }
      else{
          savePostBtn.disabled = false;
          ...
          bodyCharIcon.classList.remove('fa-exclamation-circle');
          bodyCharIcon.classList.remove('fa-times');
          bodyChar.classList.remove('has-color-red-500');
          bodyChar.classList.remove('has-color-yellow-700');
      }
  });

  $('.post_title').on('keyup change', evt => {
      evt.preventDefault();
      const $tgt = $(evt.target);
      const content = $tgt.val();
      const caretPos = content.length;
      ...
      const titleCharIcon = document.getElementById("title-char-count__icon");
      const titleChar = document.getElementById("title-char");
      const savePostBtn = document.getElementById('commit');
      titleCharCount.innerText = caretPos + '/255';

      if (caretPos > 200 && caretPos <=254){
          titleChar.style.display = "block";
          savePostBtn.disabled = false;
          titleCharIcon.classList.remove('fa-ellipsis-h');
          titleCharIcon.classList.remove('fa-times');
          titleCharIcon.classList.remove('has-color-red-500');
          titleCharIcon.classList.add('fa-exclamation-circle');
          titleChar.classList.add('has-color-yellow-700');
      }
      else if (caretPos >= 255){
          titleChar.style.display = "block";
          savePostBtn.disabled = true;
          titleCharIcon.classList.remove('fa-ellipsis-h');
          titleCharIcon.classList.remove('fa-exclamation-circle');
          titleCharIcon.classList.add('fa-times');
          titleChar.classList.remove('has-color-yellow-700');
          titleChar.classList.add('has-color-red-500');
      }
      else{
          titleChar.style.display = "none";
          savePostBtn.disabled = false;
          titleCharIcon.classList.remove('fa-ellipsis-h');
          titleCharIcon.classList.remove('fa-exclamation-circle');
          titleCharIcon.classList.remove('fa-times');
          titleChar.classList.remove('has-color-red-500');
          titleChar.classList.remove('has-color-yellow-700');
      }
    });
});
