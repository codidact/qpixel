...
let CHALLENGE_ID = location.pathname.match(/(?<=posts\/)\d+/)[0];
let leaderboard;
...
let settings = {
	...
	get groupByLanguage() { return this._get('groupByLanguage'); },
	set groupByLanguage(value) { return this._set('groupByLanguage', value); },

	get mergeVariants() { return this._get('mergeVariants'); },
	set mergeVariants(value) { return this._set('mergeVariants', value); },

	get showPlacements() { return this._get('showPlacements'); },
	set showPlacements(value) { return this._set('showPlacements', value); },
...
async function getLeaderboard(id) {
	...
	for (let i = 0; i < pagePromises.length; i++) {
		...
		for (let answerPost of page_answers) {
			...
			let { language, variant } = (full_language?.match(/(?<language>.+) \((?<variant>.+)\)/) ?? full_language?.match(/(?<language>.+)/))?.groups
...
<div class="toc">
	<div style="display:flex">
		<div class="toc--header">Leaderboards by language</div>
		<div class="has-padding-2" style="margin-left:auto">
			<label>Group by language
				<input id="group-by-lang" type=checkbox${settings.groupByLanguage ? ' checked' : ''}>
			</label>
		</div>
		<div class="has-padding-2" style="margin-left:auto">
			<label title="Shows variants of a language as the same language (E.g. Python (Cython) and Python (PyPy) will both be put under Python)">Merge variants
...
groupByLanguageInput.addEventListener('click', _ => { settings.groupByLanguage = groupByLanguageInput.checked; refreshBoard(); });
mergeVariantsInput.addEventListener('click', _ => { settings.mergeVariants = mergeVariantsInput.checked; refreshBoard(); });
showPlacementsInput.addEventListener('click', _ => { settings.showPlacements = showPlacementsInput.checked; refreshBoard(); });
...
function createRow(answer) {
	...
	row.innerHTML = `
		...
		<div class="toc--full"><p style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"><span class='username'></span> <code></code></p></div>
